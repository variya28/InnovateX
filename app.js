...
/* ========= AI Chat ========= */
const responses={
  anxious:["I hear you, sister 💜. Anxiety is exhausting. Let's try a 4-7-8 breath together — inhale 4s, hold 7s, exhale 8s. You're safe here.","That sounds heavy. Can you tell me what's on your mind? Sometimes naming the fear takes away half its weight."],
  burnout:["You're carrying so much — work, home, relationships, invisible labor. That's real, and it's exhausting. What's one thing you can let go of this week?","Burnout isn't weakness, it's a signal. Your body is asking for rest. What would 20 minutes just for YOU look like today?"],
  lonely:["Loneliness is one of the hardest feelings, and it doesn't mean anything is wrong with you 💜. Would you like to see stories from other women who feel the same in our Sisterhood community?","You reached out — that's brave. Loneliness lies to us that we're the only one. You're not. Many women here feel exactly this."],
  hormonal:["Hormones are powerful, and their effect on mood is real — PMS, postpartum, perimenopause, menopause. You're not 'crazy' or 'too much'. Tracking your cycle here can help spot patterns.","Your feelings are valid, and biology plays a real role. Be gentle with yourself today. Would a calming exercise help right now?"],
  unsafe:["Your safety matters most 💜. If you feel unsafe, please tap the 🚨 Crisis Help button — Women's Helpline 181 is free and available 24/7. I'm here with you.","Thank you for trusting me with this. You deserve to feel safe. If you're in danger, please reach out to 181 or 1091 immediately. Would you like resources on staying safe?"],
  motivation:["You're stronger than you know 🌟. Look at everything you've already survived. What's one small kind thing you can do for yourself right now?","Progress > perfection. Even opening this app is self-care. You don't have to be everything to everyone today.","Being a woman in this world takes courage every single day. You're doing it. What's one thing that made you smile this week?"],
  default:["Thank you for sharing that with me. Can you tell me a little more?","I'm here. Take your time. How long have you been feeling this way?","That takes courage to say. What would feel supportive right now — resources, an exercise, or just talking?"]
};
function botReply(text){
  ...
  else if(/hormon|pms|period|postpartum|menopause|cycle/.test(t))key='hormonal';
...
function closeResModal(){document.getElementById('resModal')?.classList.add('hidden');}
...
function openJournal(){
  ...
  openResModal(`
    ...
    <p class="muted">List 3 things you're grateful for today.</p>
    <input id="g1" placeholder="1. I'm grateful for..." style="width:100%;background:var(--card2);border:1px solid rgba(167,139,250,.2);color:var(--text);padding:.8rem;border-radius:10px;margin:.4rem 0"/>
    <input id="g2" placeholder="2. I'm grateful for..." style="width:100%;background:var(--card2);border:1px solid rgba(167,139,250,.2);color:var(--text);padding:.8rem;border-radius:10px;margin:.4rem 0"/>
    <input id="g3" placeholder="3. I'm grateful for..." style="width:100%;background:var(--card2);border:1px solid rgba(167,139,250,.2);color:var(--text);padding:.8rem;border-radius:10px;margin:.4rem 0"/>
...
function saveJournal(){
  ...
  openResModal(`<h2>🌟 Saved!</h2><p>Gratitude rewires the brain toward positivity. Come back tomorrow 💜</p>`);
}
...
function openSounds(){
  openResModal(`
    <h2>🎵 Calm Sounds</h2>
    <p class="muted">Pick a soundscape. Tap again to stop.</p>
...
function stopSound(){oscNodes.forEach(n=>{try{n.stop&&n.stop();n.disconnect();}catch(e){}});oscNodes=[];
...
function openReframe(){
  openResModal(`
    ...
    <p class="muted">Challenge a negative thought using CBT.</p>
    <label style="display:block;margin-top:1rem"><strong>1. The negative thought:</strong></label>
    <textarea id="cbt1" placeholder="e.g. I'll fail my exams..." style="width:100%;background:var(--card2);border:1px solid rgba(167,139,250,.2);color:var(--text);padding:.8rem;border-radius:10px;margin-top:.4rem;min-height:60px"></textarea>
    <label style="display:block;margin-top:1rem"><strong>2. Evidence against it:</strong></label>
    <textarea id="cbt2" placeholder="e.g. I passed last sem, I've been studying..." style="width:100%;background:var(--card2);border:1px solid rgba(167,139,250,.2);color:var(--text);padding:.8rem;border-radius:10px;margin-top:.4rem;min-height:60px"></textarea>
    <label style="display:block;margin-top:1rem"><strong>3. A balanced thought:</strong></label>
    <textarea id="cbt3" placeholder="e.g. Exams are hard, but I can prepare and do my best." style="width:100%;background:var(--card2);border:1px solid rgba(167,139,250,.2);color:var(--text);padding:.8rem;border-radius:10px;margin-top:.4rem;min-height:60px"></textarea>
    <button class="btn-primary" style="margin-top:.8rem" onclick="alert('Great work reframing! 💜 Come back whenever unhelpful thoughts show up.')">Done</button>
  `);
}

/* Stop sounds when closing modal */
const _origClose=closeResModal;
closeResModal=function(){stopSound();_origClose();};
